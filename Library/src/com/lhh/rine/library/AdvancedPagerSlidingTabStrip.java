...
//    private int tabBackgroundResId;
    ...
    public AdvancedPagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        tabWidthLen = a.getDimensionPixelSize(R.styleable.AdvancedPagerSlidingTabStrip_apTabWidthLen, tabWidthLen);
...
//        txtParams.addRule(RelativeLayout.CENTER_IN_PARENT);
    ...
    private void drawTabTextMode(Canvas canvas){
        ...
        //计算下划线宽度
        if (tabWidthLen!=0){
            float tabWidth = tabWidthLen;
            float len = currentTextViewRight - currentTextViewLeft;
            if (len!=tabWidth){
                float space = (tabWidth-len)/2;
                if (len>tabWidth){
                    currentTextViewLeft = currentTextViewLeft - space;
                    currentTextViewRight = currentTextViewRight + space;
                }else {
                    currentTextViewLeft = currentTextViewLeft + space;
                    currentTextViewRight = currentTextViewRight - space;
                }
